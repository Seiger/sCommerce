                ...
                <table class="scom-dashboard__table">
                    ...
                    @forelse($recentOrders as $order)
                        ...
                        <tr>
                            ...
                            <td class="scom-dashboard__order-cell scom-dashboard__order-cell--status" data-label="@lang('sCommerce::global.status')"><span @class(['badge', 'bg-disactive' => in_array($order->status, $unprocessedes), 'bg-progress' => in_array($order->status, $workings), 'bg-active' => in_array($order->status, $completeds), 'bg-cancelled' => in_array($order->status, $canceleds)])>{{sOrder::getOrderStatusName($order->status)}}</span></td>
    ...
    <div class="scom-dashboard">
        ...
        <div class="scom-dashboard__grid">
            <section class="scom-dashboard__card scom-dashboard__card--products-views">
                ...
                <ul class="scom-dashboard__products">@forelse($topProducts as $product)<li class="scom-dashboard__product"><div class="scom-dashboard__product-image"><img src="{{$product['cover']}}" alt=""></div><div class="scom-dashboard__product-content"><span class="scom-dashboard__product-name">{{$product['title']}}</span><span class="scom-dashboard__product-meta"><span class="scom-dashboard__product-meta-item scom-dashboard__product-meta-item--views" title="@lang('sCommerce::global.views')">@svg('tabler-eye', 'scom-dashboard__product-meta-icon') {{$product['views']}}</span>@if($product['sku'] !== '')<span class="scom-dashboard__product-meta-item scom-dashboard__product-meta-item--sku" title="@lang('sCommerce::global.sku')">@svg('tabler-barcode', 'scom-dashboard__product-meta-icon') {{$product['sku']}}</span>@endif</span><span class="scom-dashboard__product-revenue">{{sCommerce::convertPrice($product['price'], sCommerce::currentCurrency())}}</span></div></li>@empty<li class="scom-dashboard__empty">@lang('sCommerce::global.no_data_found')</li>@endforelse</ul>
            </section>
        ...
        <section class="scom-dashboard__card scom-dashboard__chart-card">
            <div class="scom-dashboard__card-head scom-dashboard__chart-head"><div><h3 class="scom-dashboard__chart-title">@lang('sCommerce::global.dashboard_views_dynamics')</h3><div class="scom-dashboard__chart-subtitle">@lang('sCommerce::global.dashboard_history_unavailable')</div></div><div class="scom-dashboard__chart-content"><div class="scom-dashboard__chart-metrics"><div class="scom-dashboard__chart-metric"><span>@lang('sCommerce::global.views')</span><strong>{{$totalViews}}</strong></div><div class="scom-dashboard__chart-metric"><span>@lang('sCommerce::global.products')</span><strong>{{$totalProducts}}</strong></div><div class="scom-dashboard__chart-metric"><span>@lang('sCommerce::global.reviews')</span><strong>{{$totalReviews}}</strong></div><div class="scom-dashboard__chart-metric"><span>@lang('sCommerce::global.dashboard_average_rating')</span><strong>{{$averageRating}}</strong></div></div><div class="scom-dashboard__chart-actions"><div class="scom-dashboard__periods">@foreach([7, 30, 90, 365] as $period)<a @class(['scom-dashboard__period', 'scom-dashboard__period--active' => $dashboardPeriod === $period]) href="{!!sCommerce::moduleUrl()!!}&get=dashboard&dashboard_period={{$period}}">{{$period === 365 ? __('sCommerce::global.year') : $period . ' ' . __('sCommerce::global.days')}}</a>@endforeach</div></div></div></div>
            <div class="scom-dashboard__chart"><canvas id="viewsChart"></canvas><div class="scom-dashboard__history-note">@lang('sCommerce::global.dashboard_history_unavailable')</div></div>
        </section>
    </div>
@endif
