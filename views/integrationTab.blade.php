<h3>{!!$item->instance->getIcon()!!} {!!$item->instance->getTitle()!!} ({!!$item->instance->getKey()!!})</h3>
<div class="split my-3"></div>

<form id="form" name="form" method="post" enctype="multipart/form-data" action="{!!sCommerce::moduleUrl()!!}&get=integrationSave" onsubmit="documentDirty=false;">
    <input type="hidden" name="back" value="&get=integration&i={{request()->integer('i')}}" />
...
@push('scripts.bot')
    <div id="actions">
        <div class="btn-group">
            <a id="Button5" class="btn btn-secondary" href="{!!$moduleUrl!!}&get=integrations{{request()->has('page') ? '&page=' . request()->page : ''}}">
                <i class="fa fa-times-circle"></i><span>@lang('sCommerce::global.to_list_integrations')</span>
            </a>
