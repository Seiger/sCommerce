@php
    ...
    $sCommerceSignatureLogo = base64_encode(file_get_contents(EVO_CORE_PATH . 'vendor/seiger/scommerce/images/scommerce.svg'));
    $sCommerceStylesVersion = substr(md5_file(EVO_CORE_PATH . 'vendor/seiger/scommerce/css/scommerce.css'), 0, 12);
@endphp
...
@section('content')
    ...
    <div class="sectionBody scommerce-module-shell">
    ...
    <aside class="scommerce-signature" data-scommerce-signature>
        <button class="scommerce-signature__toggle" type="button" aria-label="@lang('sCommerce::global.dashboard_documentation')" aria-expanded="false" data-scommerce-signature-toggle>
            <img class="scommerce-signature__logo" src="data:image/svg+xml;base64,{{$sCommerceSignatureLogo}}" alt="" aria-hidden="true">
        </button>
        <a class="scommerce-signature__link" href="https://seiger.github.io/sCommerce/getting-started/" target="_blank" rel="noopener noreferrer">
            <span>sCommerce</span>
            @svg('tabler-external-link', 'scommerce-signature__external')
        </a>
    </aside>
@endsection
@push('scripts.top')
...
@push('scripts.bot')
    ...
    <script>
        (() => {
            const signature = document.querySelector('[data-scommerce-signature]');
            const toggle = signature?.querySelector('[data-scommerce-signature-toggle]');
            if (!signature || !toggle) return;

            const setExpanded = (expanded) => {
                signature.classList.toggle('is-expanded', expanded);
            ...
            document.addEventListener('pointerdown', (event) => {
                if (!signature.contains(event.target)) setExpanded(false);
            });
        })();
    </script>
    <img src="{{asset('images/noimage.png')}}" id="img-preview" style="display: none;" class="post-thumbnail">
@endpush
