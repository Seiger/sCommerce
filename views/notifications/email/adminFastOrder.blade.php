...
@section('subject')
    {{evo()->getConfig('site_name')}} @lang('Quick order created by user') {{implode(' ', [trim($order->user_info['name'] ?? ''), trim($order->user_info['phone'] ?? ''), trim($order->user_info['email'] ?? '')])}}
@endsection
@section('content')
    <h3>
        @lang('Created new Quick order')
