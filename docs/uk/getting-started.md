...
## Оновлення відгуків до 1.4.0

Після оновлення пакета зробіть резервну копію бази та запустіть міграції з директорії
`core` командою `php artisan migrate`.

Міграція `2026_09_03_000000_add_review_details_and_photo` додає відсутні поля
`pros`, `cons`, `image` у `s_reviews` і розширює `message` до `mediumText`.
У SQLite/PostgreSQL використовується відповідний тип `text`.
Наявні значення, вручну додані поля й більший тип `longtext` зберігаються;
повторний запуск безпечний.

Відкат цієї міграції навмисно не видаляє поля й не скорочує повідомлення,
щоб не втратити відгуки або прив'язки до фото.
...
- **[Посібник розробника](./developers/index.md)** - Розширена конфігурація та налаштування
- **[Адміністративний посібник](./admin/index.md)** - Управління магазином через адмін-панель
